...
class Siam:
    ...
    def evaluate(self, test_dataset, weights_path=None):
        ...
        plt.figure()
        plt.grid()
        plt.title('tpr&fpr/threshold')
        plt.plot(thresholds, tp_test, label='tpr')
        plt.plot(thresholds, fp_test, label='fpr')
        plt.xlabel('threshold')
        plt.xticks(np.arange(0, 0.3, 0.02), rotation=270)
        plt.yticks(np.arange(0, 1, 0.1))
        plt.xlim((0, 0.3))
        plt.legend()
        plt.savefig('net_png/tpr&fpr_threshold.png')
        plt.show()
        ...
        plt.xlabel('fpr')
        plt.ylabel('tpr')
        plt.xticks(np.arange(0, 1, 0.1), rotation=270)
        plt.yticks(np.arange(0, 1, 0.1))
        plt.savefig('net_png/tpr_fpr.png')
        plt.show()
